...
const validateGraphSpec = (graphSpec: GraphSpec): boolean => {
  ...
  switch (graphSpec.graphType) {
    ...
    default:
      console.error(
        "Invalid GraphSpec: unknown graphType",
        (graphSpec as any).graphType
      );
      return false;
  }
};

const validateMathematicalGraphSpec = (
  graphSpec: MathematicalGraphSpec
): boolean => {
  if (!graphSpec.plot) {
  ...
  if (plot.domain) {
    ...
    for (const key of domainKeys) {
      if (
        plot.domain[key] &&
        (!Array.isArray(plot.domain[key]) || plot.domain[key]!.length !== 2)
      ) {
        console.error(
          `Invalid MathematicalGraphSpec: domain.${key} must be [min, max] array`
        );
        return false;
      }
    }
    ...
    switch (plot.kind) {
      case "2d_explicit":
        if (!plot.expressions.yOfX) {
          console.error(
            "Invalid MathematicalGraphSpec: 2d_explicit requires yOfX expression"
          );
          return false;
        }
        break;
      case "2d_parametric":
        if (!plot.expressions.xOfT || !plot.expressions.yOfT) {
          console.error(
            "Invalid MathematicalGraphSpec: 2d_parametric requires xOfT and yOfT expressions"
          );
          return false;
        }
        break;
      case "2d_polar":
        if (!plot.expressions.rOfTheta) {
          console.error(
            "Invalid MathematicalGraphSpec: 2d_polar requires rOfTheta expression"
          );
          return false;
        }
        break;
      case "3d_surface":
        if (!plot.expressions.surfaceZ) {
          console.error(
            "Invalid MathematicalGraphSpec: 3d_surface requires surfaceZ expression"
          );
          return false;
        }
        break;
      case "cylindrical_integral":
        if (!plot.expressions.cylindricalZ) {
          console.error(
            "Invalid MathematicalGraphSpec: cylindrical_integral requires cylindricalZ expression"
          );
          return false;
        }
        break;
      case "spherical_integral":
        if (!plot.expressions.sphericalR) {
          console.error(
            "Invalid MathematicalGraphSpec: spherical_integral requires sphericalR expression"
          );
          return false;
        }
        break;
...
const validateStatisticalGraphSpec = (
  ...
  if (
    !statistics.data ||
    (Array.isArray(statistics.data) && statistics.data.length === 0)
  ) {
...
const generateIntegralArea = (spec: MathematicalGraphSpec): THREE.Vector3[] => {
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    const lowerBound =
      typeof integral.lowerBound === "string"
        ? evaluateExpression(integral.lowerBound, integral.variable, x)
        : integral.lowerBound;
    const upperBound =
      typeof integral.upperBound === "string"
    ...
    for (let j = 0; j <= resolution; j++) {
      ...
      if (x >= lowerBound && x <= upperBound) {
        try {
          ...
          console.warn(
            `Error evaluating 3D integral function at (${x}, ${y}):`,
            error
          );
        }
      }
...
const generateCylindricalSurface = (
  spec: MathematicalGraphSpec
): THREE.Vector3[] => {
...
const generateCylindricalIntegral = (
  spec: MathematicalGraphSpec
): THREE.Vector3[] => {
    ...
    for (let j = 0; j <= resolution; j++) {
      for (let k = 0; k <= resolution; k++) {
        ...
        try {
          ...
          console.warn(
            `Error evaluating integral bounds at (r=${r}, theta=${theta}, z=${z}):`,
            error
          );
        }
      }
...
const generate3DSurfaceMesh = (
  ...
  if (!domain.x || !domain.y || !expressions.surfaceZ)
...
const calculateOptimalDomain = (
  graphSpec: MathematicalGraphSpec
): { x: [number, number]; y: [number, number]; t: [number, number] } => {
  ...
  const extendedX: [number, number] =
    xRange < minRange
      ? [
          currentX[0] - (minRange - xRange) / 2,
          currentX[1] + (minRange - xRange) / 2,
        ]
      : currentX;

  const extendedY: [number, number] =
    yRange < minRange
      ? [
          currentY[0] - (minRange - yRange) / 2,
          currentY[1] + (minRange - yRange) / 2,
        ]
      : currentY;

  const extendedT: [number, number] =
    tRange < 2 * Math.PI ? [currentT[0], currentT[0] + 2 * Math.PI] : currentT;
    ...
    try {
      ...
      if (
        ...
        if (meshData.vertices.length > 0) {
          ...
          const surfaceMaterial = materials.surface.clone();
          surfaceMaterial.color.setHex(
        ...
        if (
          ...
          if (volumePoints.length > 0) {
            ...
            const volumeMaterial = materials.area.clone();
            volumeMaterial.color.setHex(
              parseInt(
                mathSpec.plot.integral.areaColor?.replace("#", "") || "8b5cf6",
                16
              )
            );
            volumeMaterial.opacity = mathSpec.plot.integral.areaOpacity || 0.4;
        ...
        if (
          ...
          if (areaPoints.length > 0) {
            ...
            areaGeometry.setAttribute(
              "position",
              new THREE.Float32BufferAttribute(vertices, 3)
            );

            const areaMaterial = materials.area.clone();
            areaMaterial.color.setHex(
              parseInt(
                mathSpec.plot.integral.areaColor?.replace("#", "") || "3b82f6",
                16
              )
            );
            areaMaterial.opacity = mathSpec.plot.integral.areaOpacity || 0.3;
      ...
      >
        <div className="flex items-center justify-center h-full bg-gray-100">
          <div className="text-center p-4">
            ...
            <div className="text-gray-600 text-sm">
              The graph specification contains invalid data. Please check the
              console for details.
            </div>
          </div>
        </div>
    ...
    >
      <ThreeJSErrorBoundary>
        <Canvas {...canvasSettings}>
          {/* Optimized lighting - reduced number of lights for better performance */}
